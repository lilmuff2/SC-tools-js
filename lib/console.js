  ...
  for (let lineIndex = 0; lines.length > lineIndex; lineIndex++) {
    ...
    if (options.center) {
      process.stdout.write(
        " ".repeat(process.stdout.columns / 2 - line.length / 2)
      );
    }
...
function selectFromArray(namesList, descriptionList = []) {
  for (let nameIndex = 0; namesList.length > nameIndex; nameIndex++) {
    ...
    if (description) {
      text +=
        " ".repeat(Math.floor(process.stdout.columns / 2) - text.length) +
        ": " +
        description;
    }
    console.log(text);
...
function print(
  text,
  center = false,
  textColor = colours.reset,
  bgColor = colours.reset
) {
  ...
  for (let lineIndex = 0; lines.length > lineIndex; lineIndex++) {
    ...
    console.log(textColor + bgColor + line + colours.reset);
  }
}
function color(
  text,
  ...
  for (let lineIndex = 0; lines.length > lineIndex; lineIndex++) {
    ...
    return textColor + bgColor + line + colours.reset;
  }
}
