...
function print(text, center = false, textColor = colours.reset, bgColor = colours.reset) {
...
function color(text, center = false, textColor = colours.reset, bgColor = colours.reset) {
    let lines = String(text).split("\n");
    for (let lineIndex = 0; lines.length > lineIndex; lineIndex++) {
        let line = lines[lineIndex];
        if (center) {
