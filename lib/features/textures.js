...
const { SupercellSWF } = require("supercell-swf");
const { color, colours } = require("../console");

function texturesDecode(
  options = {
    inPath: constants.SC,
    outPath: constants.TEXTURES,
    output: true,
  }
) {
  options.output = true;
  ...
  fs.mkdirSync(folder);
  const infoFile = [];
  const swf = (0, utils_1.createSWF)(options.output).loadExternalTexture(
    `${options.inPath}`
  );
  (0, console_1.trace)(locale_1.locale.imageSaving, {
    textColor: console_1.colors.green,
  });
  for (let i = 0; swf.textures.length > i; i++) {
    const texture = swf.textures[i];
    fs.writeFileSync(
      `${folder}/${FileName}${"_".repeat(i)}.png`,
      texture.image.toBuffer()
    );
  ...
  if (options.output) {
    ...
    (0, console_1.trace)(locale_1.locale.done, {
      center: false,
      textColor: console_1.colors.black,
      bgColor: console_1.bgColors.green,
      localeStrings: [(0, process_1.hrtime)(startTime).join(",")],
    });
  }
  if (options.output) {
...
function textureEncode(
  ...
  let texturesInfo = dirContent.includes(`${folderName}.json`)
    ? require(`${path.resolve(folder)}/${folderName}.json`)
    : [];
  let textureIndex = 0;
  for (const file of dirContent) {
    if (file.endsWith(".png")) {
      ...
      texture.image = new image_js_1.Image(
        image.width,
        image.height,
        image.data,
        {
      ...
      if (textureInfo !== undefined) {
        ...
        if (width && height) {
          if (width !== texture.width || height !== texture.height) {
            let resize = true;
            if (options.output) {
              (0, console_1.trace)(locale_1.locale.illegalSize, {
                localeStrings: [width, height, texture.width, texture.height],
              });
...
function TexOptimization() {
  ...
  console_1.trace(locale_1.locale.done, {
    center: false,
    textColor: console_1.colors.black,
    bgColor: console_1.bgColors.green,
    localeStrings: [(0, process_1.hrtime)(startTime).join(",")],
...
function Postfixchange() {
  ...
  let swf = new SupercellSWF().loadAsset(FilePath);
  let postfix = prompt(
    color(
      locale_1.locale.InputPostfix,
      false,
      colours.fg.orange,
      colours.bg.black
    )
