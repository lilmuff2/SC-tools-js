...
function Folder2IconsSc() {
	...
	if (fs.existsSync(constants.FOLDERS+FolderName)) {
		let merge = prompt(color(locale_1.locale.Merge, false, colours.fg.orange, colours.bg.black))
		if (merge != "") {
			let name = prompt(color(locale_1.locale.InputFileName, false, colours.fg.orange, colours.bg.black))
		...
		for (let file of Folder) {
			if (file.endsWith(".png")){
				...
				swf.exports.addExport(movieclipId, `hero_icon_${file.replace(".png","")}_big`);
				...
				swf.exports.addExport(movieclipId, `hero_icon_${file.replace(".png","")}_small`);
				//movieclip pfp
				movieClip = new MovieClip();
				movieClip.framerate = 60;
				movieClip.bankIndex = swf.banks.indexOf(transformBank);
				...
				frame = new MovieClipFrame();
				let frameMatrix = new Matrix({
					a: 12.5,
					d: 12.5
				})
				transformBank.matrices.push(frameMatrix)
				frame.elements.push({
					bind: 0,
					matrix: transformBank.matrices.indexOf(frameMatrix),
				})
				...
				swf.exports.addExport(movieclipId, `player_icon_${file.replace(".png","")}`);
			}
		}
		...
		return prompt(locale_1.locale.nofile.replace("%s",constants.FOLDERS+FolderName))
	}
