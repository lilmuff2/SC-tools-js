...
function SC_DEC_CMP() {
    ...
    if (dec) {
        ...
        ScBuffer.toSc(constants.SC + file + "_old.sc", buffer[0], config_1.config.defaultCompression)
...
function getexports() {
    ...
    let pon = true
    let Exports=[]
    const Export = prompt(color(locale_1.locale.InputExportName, false, colours.fg.orange, colours.bg.black))
    if (Export == "") {
        return
    }
    swf.load(FilePath)
    ...
    while (pon){
        ...
        for(id of IDS){
            console.log("\u001B[33mID:"+id+"     "+"Export: "+swf.exports.getExportsById(id)+"\u001B[0m")
        }
        console.log("\n")
        ...
        if(op==1){
            let Export1 = prompt(color(locale_1.locale.InputExportName, false, colours.fg.orange, colours.bg.black))
            if(isNaN(Export1)){
    ...
    for (id of IDS) {
            ...
            for (frame of resource.frames) {
                for (element of frame.elements) {
                    if (element["matrix"] != undefined) {
                        element["matrix"] = swf2.banks[bankindex].addMatrix(swf.banks[resource.bankIndex].matrices[element["matrix"]])
                        if (element["matrix"] == undefined) {
                            bankindex++
                            swf2.banks.push(bank)
                    ...
                    if (element["color"] != undefined) {
                        element["color"] = swf2.banks[bankindex].addColor(swf.banks[resource.bankIndex].colors[element["color"]])
                        if (element["color"] == undefined) {
                            bankindex++
                            swf2.banks.push(bank)
                            swf2.banks[bankindex].matrices = []
        ...
        if (resource instanceof Shape) {
            ...
            for (bitmap of resource.bitmaps) {
                ...
                bitmap.textureIndex = swf2.textures.indexOf(swf.textures[bitmap.textureIndex])
            }
        }
